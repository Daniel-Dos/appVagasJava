...
export class VagasJavaService {
    ...
    getVagas() {
        return this.http.get(this.baseUrlService + 'Daniel-Dos/appVagasJava/issues').map(res => res.json());
    }
    ...
    addVaga(vagasJava: VagasJava) {
        ...
        return this.http.post(this.baseUrlService + 'Daniel-Dos/appVagasJava/issues', JSON.stringify(vagasJava), this.options)
        .map(res => res.json());
    }
}
